...
|         后置条件         | 获取测试结果                                                 |
|         主要流程         | 1. 用户启动测试程序。<br>2. 程序进行连接测试。<br>		读取配置：读取外部配置文件中的登录参数。<br>		尝试连接：尝试建立连接并返回连接结果。<br>		返回结果：将连接结果返回给用户。<br>3. 程序进行驱动切换测试。<br>		选择驱动：展示支持的数据库驱动信息供用户选择。<br>		加载外部驱动包：程序加载对应的外部驱动包。<br>		读取配置：读取外部配置文件中的登录参数及驱动类型配置。<br>		尝试连接：尝试建立连接并返回连接结果。<br>		返回结果：将连接结果返回给用户。<br>4. 程序进行字符集测试。<br>		指定测试表名：指定用于测试的数据库表名。<br>		指定测试语句的编码：读取集群中关于字符集的配置选项，并在终端中展示这些配置信息。<br>		获取表结构：获取某一表格的结构，获取每一列的字段名和字段类型。<br>		插入数据：向数据库中插入对应字符集编码的字符。<br>		返回结果：将插入和查询结果输出至终端进行比对。<br>5. 程序进行SQL语句测试。<br>		执行SQL语句：读取输入的SQL语句，并根据不同类型的SQL语句实现相应的代码。<br>		返回结果：将SQL语句的执行结果返回给用户。 |
...
​		在配置文件中，若要指定数据库所用驱动，需先将驱动jar包导入jar目录，程序在启动后会**自动扫描**jar目录下所有jar包，并列出供用户选择需要使用的驱动jar。
